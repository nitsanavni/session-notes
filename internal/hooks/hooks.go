...
- [[name]] wiki-links on the board are SIDE NOTES, resolved to "<boards-dir>/<session-id>.notes/name.md" (the user opens them with the o key in the TUI). For longer content (designs, scoping docs, research), write the file there and link it as [[name]] — do NOT link repo paths like [[docs/foo.md]]; they resolve to an empty stub note.
