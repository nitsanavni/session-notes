...
- `session-notes watch --board <path> | --session <id> [--snapshot <path>] [--once] [--interval <dur>] [--no-notes]` — block and emit the board's live external edits as a unified item diff (removed `-`, added `+`, the shape `board.DiffLines` produces). The first-class replacement for the hand-rolled `cp BOARD snap; while true; diff -U0 …` monitor loop. Polls (mtime/content, default `--interval` 2s; no fsnotify dependency) and compares the board to a snapshot **under the board lock**, so a write made via `edit --refresh-snapshot <same path>` (which refreshes the snapshot inside that same lock) is never emitted — the self-edit suppression the manual scripts hand-rolled. Also watches the board's `.notes/` sibling dir by default (reports `note added/changed/removed: <name>`; `--no-notes` disables), keyed by a `<snapshot>.notes.json` hash sidecar. `--once` exits 0 after the first change (the mode a background/`Monitor` task wants: react, then re-arm); without it, streams every change from one long-running process. Snapshots are seeded from the current state only when absent, so a change that arrives between two `--once` arms is still reported. Default snapshot is `<board>.watch`.
...
- `session-notes serve [--addr host:port] [--board <path>]` — web UI for non-tmux use (default `127.0.0.1:7080`). `/` is the dashboard (cards mirror `--dash`: liveness dot from transcript mtime, in-progress threads, urgent items, last Log line); each board is at `/b/<session-id>`, live-updating via server-sent events on the board file's mtime (500ms poll). Reads via `board.Load`; every write goes through `board.EditUnderLock` (same flock + atomic rename as the TUI/CLI). Items are addressed by section title + verbatim raw line, with a `NormalizeItemText` fallback when the line drifted cosmetically; a miss returns 409 and the client refetches. Ops: add, reply (in-thread flat semantics via `board.ReplyFlat` — shared with the TUI's `R`), fork (nests, the TUI's `F`), edit (text), status (incl. blocked), urgent, archive, delete, log (default author `user`), title, add-section (duplicate refused), rename-section (collision refused), archive-section (Archive/Log guarded), delete-section, set-content (whole-file raw replace optimistically locked on a `base` snapshot — the client's `E` editor; 409 on drift), undo, redo. `GET /api/board/{id}/raw` returns the verbatim markdown (the `E` editor's source). Undo/redo hold the last 100 web edits (full before/after content, in-memory per server); they apply only when the board still reads exactly the state the server last wrote — an intervening external write returns 409 instead of being clobbered. Side notes: `GET/POST /api/board/{id}/note/{name}` reads (seeded `# name` when missing, created only on save) and atomically writes `<id>.notes/<name>.md`; path-form names are refused — path links are never served over HTTP. The client is keyboard-first (TUI key map: `j`/`k` cursor over items and section heads, `tab`/`1`-`9` sections, `a`/`A` add, `R`/`F` reply/fork, `space`/`b`/`!` status/blocked/urgent, `e` edit/rename, `d`/`D` archive/delete, `o` link, `L` log, `E` raw editor, `u`/`ctrl+r` undo/redo, `y` path, `?` help); handled keys preventDefault so a key that opens an input never types itself into it, and the cursor follows an item across its own raw-line change by section + text (the client-side analogue of `NormalizeItemText`). The client defers re-renders while an inline input is open (mirroring the TUI's deferred reload) and parks the typed text of a failed edit in the section's add box. `m` toggles a map view mirroring the TUI's mm port: center-outward layout with mm's side balancing (heaviest-first onto the lighter side, first child claims right, document order per side), tree-structural `hjkl`, the unified enter fold cycle (default hides conversational `status: none` reply children behind a `[+N]` suffix counting only what that node hides; states that render identically are skipped), `f`/`b` focus/step-out with breadcrumb, `M` toggles Log, 40-column truncation with suffix exempt, and edit keys dispatching to the same ops; `a`/`A`/`e` are in-place — a provisional node is grafted into the map exactly where the new/edited node will live (child of the selection for `a`, the anchor's parent for `A`, the node itself for `e`) and hosts the text input inside its own box (no detached overlay), with Enter committing through the normal save path, Esc — and, for a fresh add, an arrow key — cancelling and removing the provisional node. Selection carries over both ways because map node keys equal outline stop keys. `window.__sn` exposes read-only nav + map state for the e2e suite (`test/e2e/run.sh`: fresh server + fixture board per test, failure artifacts under `test/e2e/artifacts/`). `--board` registers a board outside the boards dir and makes `/` redirect to it. Auth: loopback binds need nothing; a non-loopback `--addr` requires `--token <t>` / `$SESSION_NOTES_TOKEN` (constant-time compare; presented as `Authorization: Bearer`, `?token=` — which sets an HttpOnly cookie and redirects to a clean URL, covering SSE/fetch — or that cookie; everything is 401 without) or an explicit `--insecure` (warns). No TLS — tunnel/tailnet/proxy for untrusted networks. Not ported: opening path links.

## Op layer

Every board mutation is a named op (`board.Op`) executed by `board.Apply(b, op) (note, err)`: add (honors a leading `[x]`-style status marker in the text), reply (flat, `ReplyFlat`), fork (nests), edit, status, urgent (set, not toggle), archive, delete, log (author defaults `user`), title, add-section / rename-section / archive-section / delete-section (with the Archive/Log guards). Items are addressed by `Section`+`Raw` (verbatim line, normalized-text fallback — the web's form) or by `Query` (substring over the whole board — the CLI's form; multiple matches use the first and return an advisory note). Errors are typed: `ErrOpNotFound` (addressing failed — web 409 so the client refetches) vs `ErrOpInvalid` (malformed op — web 400). The web server and the edit CLI both dispatch through Apply, so every verb means one thing everywhere; the TUI deliberately keeps its finer-grained board-method calls — its rebase machinery replays pending mutations onto freshly-parsed trees and needs the primitives Apply is built from.
...
- Map view (`m`): the board as a center-outward mindmap (ported from mm — see
  ...
  archives, `D` deletes, `!` toggles urgent. Sections: `e` renames the heading (contents intact; a
  ...
  outline view), and `D` hard-deletes the whole section (matching the outline view's `D` and the web map);
  sections still can't be marked (`space`) from the map. On the center node `e` edits the board title (same as `T` in the list
  ...
  you (focus didn't land where your fingers expected), `S` (a TUI-dev-only key —
  `!` now toggles urgent, matching the outline and web) opens a prompt showing
